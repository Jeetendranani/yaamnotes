...
- given a two dimensional bitmap with a number of bits set, return the number of contiguous regions.
...
of xth row and yth columns as zero. Optimise it.
- Array pair sum
- generate a random 4 letter word from /usr/share/dict/words
- 1TB of data on my laptop sort the data
...
- distributed merge sort algorithm
- A file contains a billion integers, try to find any one integer that is not in the file.
- Given an array A and a number x, find the number of pairs in array A where difference of the pair elements is x.
...
queue is empty then throw an exception.
...
these smallest elements, find the largest element.
